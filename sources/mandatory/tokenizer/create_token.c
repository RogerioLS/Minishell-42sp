...
/*   Updated: 2024/07/13 14:37:20 by roglopes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*token_for_redirect(int *i, char *cmd_line)
{
	...
	if (cmd_line[*i] == meta_c)
	{
		if (cmd_line[(*i) + 1] == meta_c && cmd_line[(*i) + 2] == meta_c)
		{
			...
			token = newtoken(strndup(&cmd_line[(*i)++], 2), DLESS);
		else
			token = newtoken(strndup(&cmd_line[*i], 1), LESS);
	}
	else
		token = NULL;
	return (token);
...
t_token	*is_word_token(int *i, char *cmd_line)
{
	...
	while (ft_isspace(cmd_line[size]) == 0 && cmd_line[size])
		size++;
	token = newtoken(strndup(&cmd_line[*i], size - *i), WORD);
	*i = size;
	return (token);
}
...
t_token	*token_expanded(int *i, char *cmd_line)
{
	...
	while (ft_isalpha(cmd_line[size]) != 0 || cmd_line[size] == '_')
		size++;
	token = newtoken(strndup(&cmd_line[*i], size - *i), EXPANSION);
	*i = size;
	return (token);
}
...
int	initialize_buildtoken(char *cmd_line, t_token **token_list)
{
	...
	while (cmd_line[i])
	{
		...
		if (cmd_line[i] == '<' || cmd_line[i] == '>')
		{
			...
			token = newtoken(strndup(&cmd_line[i], 1), PIPE);
		else if (cmd_line[i] == '$')
